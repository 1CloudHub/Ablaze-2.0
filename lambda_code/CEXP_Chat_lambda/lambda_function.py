...
transcript_history_table = os.environ['transcript_history_table'] 
    ...
    try:
        ...
        if processed_question != "":
            ...
            prompt = f"""
                        ...
                        CRITICAL INSTRUTIONS:
                            - **IMPORTANT** : NEVER EVER Mention the word "Connent Xperience" or "CEXP" in the response
                            - Always respond in plain text format. Only return user-facing, concise text.
                            - DO NOT format responses using Markdown.
                            - Never start the response with "According to", "Based on the document", "The document outlines" or any similar phrasing that breaks immersion or makes the output sound like a citation
                            - Instead, write responses naturally and confidently, weaving the retrieved information into fluent sentences.
                        """
            ...
            for item in response['body']:
                if "chunk" not in item or "bytes" not in item["chunk"]:
                    continue

                try:
                    content = json.loads(item["chunk"]["bytes"].decode("utf-8"))
                except json.JSONDecodeError:
                    continue

                if "contentBlockDelta" in content:
                    delta = content["contentBlockDelta"]["delta"]

                    if "text" in delta:
                        streamed_content += delta["text"]

                        # Stream partial text to frontend (like Claude)
                        res = api_gateway_client.post_to_connection(
                    ...
                    metrics = content["metadata"].get("usage", {})
                    iti_input_tokens = metrics.get("inputTokens", 0)
                    iti_output_tokens = metrics.get("outputTokens", 0)


            final_ai_answer = streamed_content
            return {
...
def agent_invoke_tool(message_history, session_id, connectionId, last_inserted_id_value, org_question, model_id):
    ...
    try:
        ...
        while model_loop_limit < 3:
            model_loop_limit += 1
            response = bedrock_client.invoke_model_with_response_stream(
                modelId=model_id,  # e.g., "amazon.nova-pro-v1:0"
                contentType='application/json',
                body=json.dumps({
                    "schemaVersion": "messages-v1",
                    "system": [
            ...
            for item in response['body']:
                ...
                if "contentBlockDelta" in content:
                    ...
                    if "toolUse" in delta:
                        if len(assistant_response) > 0 and 'toolUse' in assistant_response[-1]:
                            assistant_response[-1]['toolUse']['input'] = json.loads(delta['toolUse']['input']) if type(delta['toolUse']['input']) != dict else delta['toolUse']['input']

                elif "contentBlockStart" in content:
                    if 'start' in content['contentBlockStart'] and 'toolUse' in content['contentBlockStart']['start']:
                        assistant_response.append(content['contentBlockStart']['start'])
                    ...
                    individual_input_tokens = metrics.get("inputTokens", 0)
                    individual_output_tokens = metrics.get("outputTokens", 0)


            final_ai_response = streamed_content
            assistant_response.append({"text" : final_ai_response})
            message_history.append({'role': 'assistant', 'content': assistant_response[::-1]})
            ...
            values = (json.dumps(assistant_response[::-1]),str(individual_input_tokens),str(individual_output_tokens),str(last_inserted_id_value))          
            ...
            for action in tool_calls:    
                tool_name = action['toolUse']['name']
                tool_input = json.loads(action['toolUse']['input']) if type(action['toolUse']['input']) != dict else action['toolUse']['input']
                tool_use_id = action['toolUse']['toolUseId']
...
def lambda_handler(event, context):
    ...
    if  session_id == 'null' :
        ...
        if history_response:
            ...
            if any("toolResult" in i for i in earliest):
    ...
    insert_query = f'''
            ...
            VALUES( %s, %s, %s, %s, %s, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING id;   
            '''
    values = (str(session_id),json.dumps([{'text': user_question}]), str(''), str('0'), str('0'))
    last_inserted_id_value = last_inserted_id(insert_query, values)    
